 ...
 * @@Modify Date: 2018-06-11 22:17:47
 * @@Function:
 */

class Magiccart_Alothemes_Adminhtml_InstallController extends Mage_Adminhtml_Controller_Action
{

    protected function _initAction() 
    {
        $this->loadLayout()
                ->_setActiveMenu('magiccart/alothemes_install')
    ...
    public function editAction() 
    {
        ...
        $this->_addContent($this->getLayout()->createBlock('alothemes/adminhtml_install_edit'))
            ->_addLeft($this->getLayout()->createBlock('adminhtml/system_config_switcher'))
            ->_addLeft($this->getLayout()->createBlock('alothemes/adminhtml_install_edit_tabs'));
        ...
        if ($data = $this->getRequest()->getPost()) {
            ...
            try 
            {
                if ($action == '1') {
                    ...
                    $restore = Mage::getSingleton('alothemes/import_cms');
                    if($data['pages']=='1')     $restore->importCmsItems('cms/page', 'page', $data['theme'], $data['overwrite_pages'], $stores);
                    if($data['blocks']=='1')    $restore->importCmsItems('cms/block', 'block', $data['theme'], $data['overwrite_blocks'], $stores);
                    ...
                    if($data['pages']=='1') Mage::getSingleton('alothemes/import_cms')->deleteCmsItems('cms/page', 'page', $data['theme'], $stores);
                    if($data['blocks']=='1') Mage::getSingleton('alothemes/import_cms')->deleteCmsItems('cms/block', 'block', $data['theme'], $stores);
                    if($data['config']=='1') Mage::getSingleton('alothemes/import_cms')->deleteSystemConfig('core/config', 'system', $data['theme'], $stores);
                }
